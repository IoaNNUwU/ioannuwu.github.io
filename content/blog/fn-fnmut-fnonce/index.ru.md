...
Это означает, что замыкание, которое реализует `Fn` может быть передано как аргумент вместо `FnOnce` или `FnMut`:

```rust
fn call_once(func: impl FnOnce()) {
    func();
}
let fn_closure /*: impl Fn() */ = || println!("Hello World");

call_once(fn_closure);
```

Вопрос "**почему так?**" будет рассмотрен далее, но сначала можно рассмотреть то, как именно компилятор присваивает трейты:
...
Существует 3 места где есть понятие об этих трейтах.

### Момент присвоения трейта компилятором

<!-- LESS RESTRICTIVE -->

Компилятор пытается как можно точнее описать ваше замыкание во время инициализации. При этом компилятор не думает о том, как это замыкание будет применено в будущем, он выбирает трейт опираясь только на тело замыкания.

Он присваивает трейты в таком порядке:

`Fn` -> `FnMut` -> `FnOnce`

Сначала компилятор пытается присвоить `Fn`. Если замыкание изменяет какое-либо состояние, то он вынужден присвоить `FnMut`. Если замыкание поглощает состояние, то он вынужден присвоить `FnOnce`.

Такой порядок ведёт к **бóльшим возможностям применения** замыканий. Если компилятор присвоил `Fn`, такое замыкание может быть передано вместо `FnMut` и `FnOnce`, а значит с точки зрения использования, `Fn` замыкания самые универсальные.
...
### Момент когда автор библиотеки выбирает, какой трейт он хочет видеть как аргумент

Автор библиотеки должен выбирать трейты в обратном порядке:

`FnOnce` -> `FnMut` -> `Fn`

Это нужно делать по той же причине - чтобы добиться **наименьших ограничений на переданное замыкание**. Если функция принимает `FnOnce` как аргумент, то туда может быть передана любое замыкание, в том числе `FnMut` и `Fn`, а значит выбирая трейты в таком порядке, автор библиотеки добивается большего удобства своей библиотеки.

Проблема в том, что применить `FnOnce` можно не всегда. Если функция будет вызвана несколько раз, то автор библиотеки вынужден применять `FnMut`. В таком случае туда может быть передана функция `FnMut` или `Fn`.

```rust
fn call_mut(func: impl FnMut()) {
    for i in 0..10 {
        func();
    }
}
let fn_closure = || println!("Hello World");

let mut text = String::from("Hello World");
let fn_mut_closure = || text.push('!');
```

`Fn` не нужно применять почти никогда, за редким исключением, связанным зачастую с многопоточностью.

### Пользователь выбирает трейт??

Единственный момент когда пользователю нужно думать о том, какой трейт реализует замыкание и почему - это если компилятор выдал ошибку.

Давайте рассмотрим стандартные ошибки:
